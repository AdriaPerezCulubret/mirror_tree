...
The MirrorTree technique is based on co-evolutionary methodology, as MirrorTree assumes that if a physical interaction exists between two proteins, they should co-evolve and moreover, they should share similar evolutionary history.


\section{Methods}
\subsection{Using mirror trees to infer interactions}
The MirrorTree methodology, developed by Dr. Alfonso Valencia and associates, predicts direct interactions between proteins based on co-evolution of interacting proteins. \cite{Pazos2001}. The work-flow of the MirrorTree is as follows:
\begin{enumerate}
\setlength{\itemsep}{1pt}
	...
	\item Compare the distance matrices at 1:1 rate.
	\item Compute a correlation coefficient (Pearson's r) based on the difference between the matrices.
...
\subsection{mtree}
Our integrative tool (\texttt{mtree}) takes a slightly different approach to the Valencia's team to MirrorTrees. Our pipeline is as follows:
\begin{enumerate}
\setlength{\itemsep}{1pt}
	...
	\end{enumerate}
	\item Look for orthologous sequences for each query sequence (search with  hidden markov models, using jackhmmer \cite{HMMER}).
	\item Build an MSA for each query sequence and their putative orthologs (using hmmalign \cite{HMMER}).
	\item Use the distance matrix for each set provided by jackhmmer at the last iteration \cite{HMMER} (using Blosum62 matrix).
	...
	\item Infer interaction.
	\item Create an interactome (graph).
\end{enumerate}
