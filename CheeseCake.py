...
def run_jackhmmer(query_dict, target_dict, query, target, verbose):
    ...
    if verbose:
        ...
        sys.stderr.flush()
    os.system("jackhmmer -E 1e-5 --tblout tmp/jackhmmer.tbl -N 1 --chkhmm tmp/chkhmm %s %s > /dev/null" % (query, target))
...
for seq in itertools.combinations(query_dict.keys(), 2):
    ...
    if len(common_sp) >= options.species:
        ...
        sys.stderr.flush()
        correlation_list.append([seq1.id, seq2.id, interaction.get_corr()[0]])
